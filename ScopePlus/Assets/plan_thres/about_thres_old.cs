...
public class about_thres_old : MonoBehaviour {
    ...
    float[] p_ori;
    GameObject[] sp, sp_2;
    Vector3[] pos_sp, pos_sp2;
    public Camera maincamera;
    
    Vector3 pos_l, pos_r;
    Texture2D tex, tex_warp;
    public Texture2D tex_ori, tex_ori2; //1為上翅 2為下翅
    public GameObject wingUL,wingDL,wingUR,wingDR;
	...
	void Update () {
        ...
        if (Input.GetKey(KeyCode.Space))
        {
            ...
            if (CheckBoudary(pos_sp, pos_l, pos_r) && CheckBoudary(pos_sp2, pos_l, pos_r)) //check boundary
            {

                tex_warp = warp(tex_ori, tex, P_ori, P_def);//左
                SetColorToTransWings(tex_warp, tex_ori, wingUL, wingUR); //右上左上翅
                tex_warp = warp(tex_ori2, tex, P_ori, P_def2);//右
                SetColorToTransWings(tex_warp, tex_ori2, wingDL, wingDR);//右下左下翅
    ...
    void SetColorToTransWings(Texture2D tex_warp, Texture2D tex_ori, GameObject wingL, GameObject wingR)
    {
